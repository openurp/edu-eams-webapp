...
class GradeRateServiceImpl extends BaseServiceImpl with GradeRateService {
  ...
  def calcGp(grade: CourseGrade): java.lang.Float = {
    ...
    if (null != conifg) {
      var gp = calcGp(grade.getScore, conifg)
      if (null != gp && gp.floatValue() > 1 && null != grade.getScore && 
        grade.getScore < 61) {
        if (null != 
      ...
      while (iter.hasNext) {
        ...
        if (gradeRateItem.inScope(score)) {
          if (Strings.isNotEmpty(gradeRateItem.getGpExp)) {
            ...
            return expressionEvaluator.eval(gradeRateItem.getGpExp, data, classOf[Float])
          } else {
            return null
          }
        }
  ...
  def isPassed(score: java.lang.Float, scoreMarkStyle: ScoreMarkStyle, project: Project): Boolean = {
    ...
    if (null == config || null == score) {
      ...
      java.lang.Float.compare(score, config.getPassScore) >= 
        0
    }
  }
