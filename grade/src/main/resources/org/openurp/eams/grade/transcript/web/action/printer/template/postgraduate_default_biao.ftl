...
<table align="center" class="zgPrintTable">
	    ...
	    <tr align="center">
	    <td>${(grade.lesson.courseType.name)!}</td><td colspan="2">${(grade.getCourse().getName())!}</td><td>${(grade.getCourse().credits)!}</td><td>${(grade.score)!}</td><td>${(grade.lesson.semester.beginOn?string("yyyy-MM"))!}</td><td>${(grade.remark?html)!}</td>
	    </tr>
	         [/#list]
	    <tr>
	    <tr align="center">
