...
<BODY style="margin:0px;"f_frameStyleResize(self)'>
 ...
 <div id = "PrintA" width="100%" align="center" cellpadding="0" cellspacing="0">
   <#assign tableIndex=0/>
    <#list multiStdGpas as multiStdGpa>
    <#if multiStdGpa.stdGpas?exists>
    <#assign stdGpas = multiStdGpa.stdGpas?sort_by(["std","code"])/>
    <#assign semesters = multiStdGpa.semesters?sort_by("schoolYear")/>
    <#list stdGpas?chunk(pageSize) as stdGpaList>
 	 <table width="100%" style="border:0;<#if tableIndex!=0>PAGE-BREAK-before: always;</#if>" cellpadding="0" cellspacing="0">
 	    <tr>
 	      <td align="center" colspan="20"><h3><@i18nName stdGpa.std.project.school! />班级绩点一览表</h3></td>
 	    </tr>
 	 	<tr>
 	 	  <td style="font-size:15px;" colspan="20">
 	 	  <#if multiStdGpa.adminClass?if_exists.name?exists>
 	 	  	班级:<@i18nName multiStdGpa.adminClass/>
 	 	  </#if>
 	 	  </td>
 	 	</tr>
	 </table>
     ...
     <table width="100%" align="center" class="listTable">
	   <tr class="title" align='center'>
	     ...
	     <td style="width:80px"><@text name="attr.name"/></td>
	     <td>平均绩点</td>
	     <#list semesters as semester>
	     <td >${semester.schoolYear} ${semester.name}</td>
	     </#list>
	   </tr>
	   <#list stdGpaList as stdGpa>
	    <tr align="center">
		   <td>${stdGpa.std.code}</td>
		   <td><@i18nName stdGpa.std/></td>
		   <td>${stdGpa.gpa!}</td>
		   <#list semesters as semester>
		   <td>${(stdGpa.getGpa(semester))!}</td>
		   </#list>
	   </#list>
     </table>
  </#list>
