...
<table width="100%" style="border-collapse: collapse;border:solid;border-width:0px;"><tr><td>${b.text('attr.taskNo')}:${lesson.no!} ${b.text('attr.courseName')}:${lesson.course.name!}[#if lesson.subCourse??]<sup style="color:#29429E">${lesson.subCourse.name}</sup>[/#if]</td>[#list gradeStatus?keys as ss]<td style="background-color:${gradeStatusColor[ss]};text-align:center" width="80px">${gradeStatus[ss]}</td>[/#list]</tr></table>
[#if gradeTypes?size>0][#assign gradeWidth=47/(gradeTypes?size+1)][#else][#assign gradeWidth=52/] [/#if]
[@b.grid items=grades var="courseGrade" sortable="false"]
	[@b.row]
		[@b.boxcol/]
		[@b.col width="4%" title="attr.index"]${courseGrade_index+1}[/@]
		[#if gradeTypes?size==0]
			[@b.col width="25%" property="std.code" title="attr.stdNo"/]
			[@b.col width="22%" property="std.name" title="attr.personName"/]
			[@b.col width="36%" property="courseTakeType.name" title="修读类别"/]
		[#else]
			[@b.col width="15%" property="std.code" title="attr.stdNo"/]
		...
		[#list gradeTypes as gradeType]
			...
			[#if examGrade!="null"]
				...
				[/@]
			[#else]
				[@b.col title=gradeType.name width="${gradeWidth}%" property="gradeType.${gradeType.id}"][/@]
			[/#if]
		[/#list]
	[@b.col width="${gradeWidth}%" property="scoreText" title="最终成绩" style="background-color:${gradeStatusColor[courseGrade.status?string]}"]
