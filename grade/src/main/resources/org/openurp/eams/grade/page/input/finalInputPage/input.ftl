	...
	<tbody>
		...
		<tr class="griddata-even std-grades-${std.id}">
			...
			[#list gradeTypeRange as gradeType]
				...
				[/#if]
			[/#list]
			<td class="result-area [#if !(stdId2CourseGrade[std.id?string].passed)!(false)]grade-input-unpassed[/#if]">
			${(stdId2CourseGrade[std.id?string].scoreText)!}
			</td>
		</tr>
		[/#list]
	</tbody>
...
jQuery(function() {
	...
	*/
	...
	new StdGradeView({
		...
		"lessonId" : ${lesson.id},
		"saveAjaxURL" : "${b.url("!saveFinalAjax")}",
	...
	jQuery(".grade-input-submit-button").click(function(event) {
		...
		if (result.passed) {
			bg.Go("${b.url("!submitFinal?lesson.id=" + lesson.id)}");
		} else {
			alert("请录入成绩后再提交");
		}
	});
