...
<table align="center" style="text-align:center" cellpadding="0" cellspacing="0">
        <tr>
            ...
            [#if queryGradeTypes?? && queryGradeTypes?size > 0][#list queryGradeTypes as grt][@i18nName grt /][#if grt_has_next],[/#if][/#list][#else]成绩[/#if]登记表
       		<td>
        </tr>
    </table>
    <table width='95%' class="reportTitle" align='center'>
        <tr>
        	<td width="30%">课程名称:[@i18nName report.lesson.course!/][#if report.lesson.subCourse??]<sup>${report.lesson.subCourse.name}</sup>[/#if]</td>
...
[#if report.courseGrades[objectIndex]??]
    ...
    <td style="font-size:11px">${courseGrade.std.name!}[#if courseGrade.courseTakeType?exists && courseGrade.courseTakeType.id != TAKE_TYPE_NORMAL]<sup>${courseGrade.courseTakeType.name}</sup>[/#if]</td>
    
    [#list report.gradeTypes as gradeType]
    <td>
    [#local examGrade=courseGrade.getExamGrade(gradeType)!"null"/]
    [#if examGrade!="null"]
    [#if !examGrade.markStyle.numStyle && (examGrade.scoreText!)?length>2]<span class="longScoreText">${examGrade.scoreText!}</span>[#else]${examGrade.scoreText!}[/#if][#if examGrade.examStatus?? && examGrade.examStatus.id!=EXAM_STATUS_NORMAL]<span class="examStatus"> ${examGrade.examStatus.name}</span>[/#if]
    [/#if]
    </td>
   	[/#list]
[#else]
...
[#if report.courseGrades[objectIndex]??]
    ...
    <td class="columnIndex">${objectIndex + 1}</td>
    <td>${courseGrade.std.code!}</td>
    <td>${courseGrade.std.name!}[#if courseGrade.courseTakeType?exists && courseGrade.courseTakeType.id != TAKE_TYPE_NORMAL]<sup>${courseGrade.courseTakeType.name}</sup>[/#if]</td>
