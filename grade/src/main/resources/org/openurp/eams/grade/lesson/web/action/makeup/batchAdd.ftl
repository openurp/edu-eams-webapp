[#ftl]
[@b.head/]
[@b.toolbar title="${b.text('grade.course.makeup.AddStudentAchievement')}"]
...
[@b.form name="actionForm"]
	...
	<table class="infoTable" width="100%">
	   ...
	   <tr>
	   		<td class="title">${b.text("grade.course.makeup.AchievementCategory")}:</td>
	   		<td>${examType.name}</td>
	   		<td class="title">${b.text("std.majorType")}:</td>
	   		<td>
	   			<select name="majorType.id" style="width:100px">
	   				<option value="1" [#if Parameters['courseGrade.majorType.id']?if_exists="1"]selected[/#if]>${b.text("entity.firstMajor")}</option>
   	      			<option value="2" [#if Parameters['courseGrade.majorType.id']?if_exists="2"]selected[/#if]>${b.text("entity.secondMajor")}</option>
	   			</select>
	   		</td>
	...
	[@b.grid items=0..stdCount-1 var="i"]
		[@b.row]
			...
			[@b.col width="10%" title="std.score"][/@]
			[@b.col width="10%" title="${b.text('std.score')}"][/@]
			[@b.col width="10%" title="${b.text('attr.taskNo')}"][/@]
			[@b.col width="10%" title="${b.text('entity.courseType')}"][/@]
			[@b.col width="10%" title="${b.text('grade.attendingClasses')}"][/@]
			[@b.col width="10%" title="${b.text('exam.situation')}"][/@]
			[@b.col width="20%" title="${b.text('entity.explain')}"][/@]
		[/@]
	[/@]
