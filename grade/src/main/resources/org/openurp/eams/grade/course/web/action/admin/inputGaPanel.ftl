...
<div class="dynamic-tab-pane-control tab-pane" id="tabPane2">
    ...
    <div style="display: block;height:300px;padding:5px;font-size:14px;background-color:#E1ECFF;border-color:LightSkyBlue" class="tab-page" id="tabPage2_1">
        ...
        <table width="100%" height="25px">
            <tr>
            	[@large_stateinfo status=(gradeState.getState(GA).status)!0/]
                <td align="right">[#if (gradeState.getState(GA).inputedAt)??]上次录入:${gradeState.getState(GA).inputedAt?string("yyyy-MM-dd HH:mm")}[/#if]</td>
            </tr>
        </table>
            ...
            <tr valign="top">
                <td width="20%">
                    <table cellpadding="0" cellspacing="0" >
                        <tr>
                            [@gradeInfoHTML url="${b.url('/teach/grade/lesson/report!blank?lesson.id='+lesson.id)}" onclick="" tdStyle1="" tdStyle2="text-align:left" caption="空白登分表" iconSize="25px" width1="" width2="100px"/]
                        </tr>
                        [#--
                    ...
                    <table cellpadding="0"  align="center"  >
                    	[#list gaGradeTypes?sort_by("code") as gradeType]
	                     	<tr>
	                            ...
	                            <td>％</td>
	                            <td>
	                            	[@b.select  name="examGradeState${gradeType.id}.scoreMarkStyle.id" items=markStyles value=(gradeTypeState.scoreMarkStyle.id)?if_exists style="width:150px"/]
	                            </td>
	                            <td style="color:blue">
...
<script>
    ...
    function gaInput() {
        ...
        [/#if]
        form2.target = "_self";
        bg.form.submit("actionForm2","${b.url('admin-ga!input')}");
    }
    ...
    function gaPrint() {
        form2.target = "self";
       	bg.form.submit("actionForm2","${b.url('!report')}");
    }
        
    //打印分段统计
    function printStatReport(kind) {
        form2.action = "${b.url('/teach/grade/lesson/report!stat')}";
        if (null != kind) {
