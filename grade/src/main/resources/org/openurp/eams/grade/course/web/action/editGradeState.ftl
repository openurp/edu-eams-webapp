...
[#assign gradeStatus={'0':'新添加', '1':'已提交', '2':'已发布'}/]
    [@b.toolbar title="成绩状态"]
		...
		bar.addBack();
    [/@]
    [@b.form name="collegeGradeEditForm"  target="contentDiv" action="${actionURL}"]
		...
		<table  class="formTable" width="100%" id="gradePercent" >
			<tr>
				<td colspan="7">各种成绩录入百分比和发布状态,保存时请选中左侧复选框。</td>
			</tr>
	        ...
	        <tr align="center">
	            ...
	            <td>[#assign stateName]state${state.gradeType.id}.scoreMarkStyle.id[/#assign]
		            ...
		            [#else]
		            	[@b.select items=markStyles value=state.scoreMarkStyle.id?if_exists name=stateName/]
		            [/#if]
	            </td>
	            <td>[#if noPercentIds?seq_contains(state.gradeType.id)]
	            	[#else]
		            	[#if gradeState.status==2]
	            ...
	            [#else]
		            [@b.select name="state${state.gradeType.id}.precision"  style="width:100px;" items={'0':'保留整数','1':'保存一位小数'}/]
		        [/#if]
	            </td>
	            [#if (state.id)?exists]
	            ...
	            [/#if]
	            <td>
	            	<select [#if gradeState.status==2]disabled[/#if] name="state${state.gradeType.id}.status" style="width:80px">
