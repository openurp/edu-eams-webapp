		...
		<tr class="griddata-even std-grades-${std.id}">
			...
			[#list gradeTypeRange as gradeType]
				...
				[#if contains(inputableGradeTypes, gradeType)]
					...
					[#if !(examGrade.id)??]
						[#assign examGrade = stdId_gradeTypeId2examGrade[std.id?string + "-" + gradeType.id?string]! /]
						<td>${(examGrade.examType.name)!}</td>
						<td>${(examGrade.examStatus.name)!}</td>
						<td>
						[#if (examGrade.examStatus.needAttendDelay)!(false)]
						无考试记录<br>不能录入
						[#else]
						${(examGrade.scoreText)!}
						[/#if]
				...
				[/#if]
			[/#list]
			<td class="result-area [#if !(stdId_gradeTypeId2examGrade[std.id?string + "-" + GRADE_TYPE_GA.id?string].passed)!(false)]grade-input-unpassed[/#if]">
			${(stdId_gradeTypeId2examGrade[std.id?string + "-" + GRADE_TYPE_GA.id?string].scoreText)!}
			</td>
		</tr>
		[/#list]
	</tbody>
...
jQuery(function() {
	...
	*/
	...
	new StdGradeView({
		...
		"lessonId" : ${lesson.id},
		"saveAjaxURL" : "${b.url("!saveGADelayAjax")}",
	...
	jQuery(".grade-input-submit-button").click(function(event) {
		...
		if (result.passed) {
			bg.Go("${b.url("!submitGADelay?lesson.id=" + lesson.id)}");
		} else {
			alert("请录入成绩后再提交");
		}
	});
