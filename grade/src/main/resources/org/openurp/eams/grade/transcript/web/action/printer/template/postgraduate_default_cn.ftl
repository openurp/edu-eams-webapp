...
<table align="center" class="zgPrintTable">
	    ...
	    [#list grades.get(std) as grade]
	     <tr align="center">
	        <td colspan="4">${(grade.course.name)!}</td><td>${(grade.score)!}</td><td>${(grade.course.credits)}</td><td>${getCompulsory(planCourses.get(std))}</td><td>${notCompulsory(planCourses.get(std))}</td><td>${(grade.course.remark?html)!}</td>
	    </tr>
	    [/#list]
