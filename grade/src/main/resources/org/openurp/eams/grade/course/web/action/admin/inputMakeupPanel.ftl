    ...
    <input type="hidden" name="gradeTypeIds" value="${MAKEUP.id},${DELAY_ID}"/>
        ...
        <div style="display: block;height:100px;padding:5px;font-size:14px;background-color:#E1ECFF;border-color:LightSkyBlue;" class="tab-page" id="tabPage3_1">
              ...
              <tr valign="top">
                <td width="20%">
                    <table cellpadding="0" cellspacing="0" >
                        <tr>
                            [@gradeInfoHTML url="${b.url('/teach/grade/lesson/report!blank?makeup=1&lesson.id=' +lesson.id)}" onclick="" tdStyle1="" tdStyle2="text-align:left" caption="空白登分表" iconSize="25px" width1="" width2="100px"/]
                        </tr>
                    </table>
                </td>
                <td>
            <table align="center" style="font-size:14px;text-align:center" cellpadding="0" cellspacing="0">
                <tr height="35px">
                    [@gradeInfoHTML url="${b.url('admin-makeup!info')}?lessonId=${lesson.id}" onclick="" tdStyle1="" tdStyle2="text-align:left" caption="查看" iconSize="25px" width1="" width2="40px"/]
...
<script>
    ...
    function makeupInput() {
       	bg.form.submit("actionForm3","${b.url('admin-makeup!input')}");
    }
    
    function makeupRemove() {
        if (confirm("要删除该课程该类型成绩吗？")) {
           	bg.form.submit("actionForm3","${b.url('admin-makeup!removeGrade')}");
        }
    }
   
    function makeupPrint() {
        bg.form.submit("actionForm3","${b.url('admin-makeup!report')}");
    }
</script>