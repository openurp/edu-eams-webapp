...
<table align="center" class="zgPrintTable">
	    ...
	    [#list grades.get(std) as grade]
	     <tr align="center">
	        <td>${(grade.getCourse().getEngName())!}</td><td> ${(grade.score)!}</td><td>${(grade.getCourse().credits)!}</td><td>${getCompulsory(planCourses.get(std))}</td><td>${notCompulsory(planCourses.get(std))}</td><td>${(grade.course.remark)!}</td>
	    </tr>
	    [/#list]
	     <tr>
