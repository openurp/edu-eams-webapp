...
<td id="TD_${(gradeType.id)!}_${courseTake.std.id}">
...
[/#if]
[#--根据策略是否显示输入框--]
[#if gradeTypePolicy.shouldHaving(courseTake,gradeType,examTake)]
	[#--判断不能录入考试分数的情况--]
	...
	[#if !couldInput]
		<input type="hidden" value="" id="${(gradeType.id)!}_${index + 1}" name="${(gradeType.id)!}_${courseTake.std.id}"/>
	...
	[#if (grade.getExamGrade(gradeType))??] [#local examGrade=grade.getExamGrade(gradeType)/][/#if]
	    ...
	    [@b.select items=examStatuses value=((examGrade.examStatus)!examStatus) name="examStatus_" + (gradeType.id)! + "_" + courseTake.std.id id="examStatus_" + (gradeType.id)! + "_" + (index + 1) style="width:60px;" 
...
[#macro displayGrades(index, courseTake)]
    ...
    <td>
    	...
    	[#list gradeTypes?sort_by('code') as gradeType]
    		 ...
    		 [/#if]
    	[/#list]
    	[#--<a href="#" onClick="setPersonPercent('${courseTake.std.id}','${gradeTypeIdSeq}','${(grade.id)!}')" title="设置个人百分比">--]
    	${courseTake.std.person.name}
    	[#--</a>--]
    ...
    [#list gradeTypes?sort_by('code') as gradeType]
    [#if gradeType.id==GA.id]
    <td align="center" id="GA_${index + 1}">[#if grade?exists]${(grade.getGrade(GA).scoreText)!}[/#if]</td>
    [#else]
        [#if (grade.getGrade(gradeType).confirmed)!false]
			<td>${(grade.getGrade(gradeType).scoreText)!}</td>
        [#else]
...
${lesson.semester.schoolYear!}学年${(lesson.semester.name)?if_exists?replace('0','第')}学期
        ...
        <tr style="background-color: #FFFFBB">
       	...
       	<tr style="background-color: #FFFFBB">
            ...
            <td>
	            ...
	            <input type="radio" name="inputTabIndex" value="1" checked>
	            ${b.text("grade.recordModeByStd")}
	            <input type="radio"  name="inputTabIndex" value="0">
    ...
    <table class="gridtable" style="width:98%" align="center">
        ...
        [#list 1..2 as i]
            ...
            <td width="50px">${b.text("attr.studentName")}</td>
            [#list gradeTypes?sort_by('code') as gradeType]
    ...
    <table width="100%" style="font-size:15px" height="70px">
        <tr>
            ...
            [#if (canInputedCount > 0 && lesson.teachClass.courseTakes?size > 0)]
              [@b.submit value="暂存" onsubmit="saveGrade(true)" title="${b.text('grade.remaindInputNextTime')}" id="bnJustSave"/] 
              &nbsp;&nbsp;&nbsp;[@b.submit value="提交" onsubmit="saveGrade(false)" id="bnSubmit"/]
            [/#if]
            </td>
        </tr>
    </table>
