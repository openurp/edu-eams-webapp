	  ...
	  <tbody>
	  	...
	  	[#if (courseGrades.size()/2-1)>0 || (courseGrades.size()/2-1)=0]
	  		[#list 0..(courseGrades.size()/2-1) as len]
	  			...
	  			<td>[@i18nName courseGradeFir.course!/][#if courseGradeFir.courseTakeType?? && courseGradeFir.courseTakeType.id == RESTUDY](重修)[/#if]</td>
	  			<td>${courseGradeFir.course.credits!}</td>
	  			<td>${courseGradeFir.scoreText!}</td>
	  			<td>${courseGradeFir.gp!}</td>
	  			<td>${stdSemMap[(courseGradeFir.semester.id)+'']!}</td>
	  			<td>${courseGradeFir.remark!}</td>
	  			<td>[@i18nName courseGradeSec.course!/][#if courseGradeSec.courseTakeType?? && courseGradeSec.courseTakeType.id == RESTUDY](重修)[/#if]</td>
	  			<td>${courseGradeSec.course.credits!}</td>
	  			<td>${courseGradeSec.scoreText!}</td>
	  			<td>${courseGradeSec.gp!}</td>
	  			<td>${stdSemMap[(courseGradeSec.semester.id)+'']!}</td>
	  	...
	  	[#if courseGrades.size()%2!=0]
	  		[#assign courseGrade = courseGrades[courseGrades.size()-1]/]
		  		<tr>
		  			<td>[@i18nName courseGrade.course!/][#if courseGrade.courseTakeType?? && courseGrade.courseTakeType.id == RESTUDY](重修)[/#if]</td>
		  			<td>${courseGrade.course.credits!}</td>
		  			<td>${courseGrade.scoreText!}</td>
		  			<td>${courseGrade.gp!}</td>
		  			<td>${stdSemMap[(courseGrade.semester.id)+'']!}</td>
...
<table width="95%" align="center" class="reportTable" valign="top" style="font-size:11px">
 	<tbody>
	 		<tr style="text-align:center;background-color:#C7DBFF;">
	 			[#list 1..3 as i]
