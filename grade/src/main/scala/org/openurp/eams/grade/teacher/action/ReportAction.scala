...
import org.openurp.eams.grade.teacher.util.GradeSegStats
import org.openurp.eams.grade.teacher.util.FloatSegment

class ReportAction extends AbstractEntityAction {
  def index(): String = {
    val lessonId = 1
    val lesson = entityDao.findBy(classOf[Lesson], "id", List(lessonId))
    val grades = entityDao.findBy(classOf[CourseGrade], "lesson.id", List(lessonId))
    ...
    grades.foreach(grade => {
      lglist.foreach(lg => {
        val a = grade.getGrade(gradeType)
        if (a!=null){
        	val score = a.score
  ...
  def statTask(): String = {
    val lessonId = 1
    val lesson = entityDao.findBy(classOf[Lesson], "id", List(lessonId)).head
    val grades = entityDao.findBy(classOf[CourseGrade], "lesson.id", List(lessonId))
