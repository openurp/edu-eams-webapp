 ...
 * @author chaostone
 */
class TeacherAction extends ActionSupport {
  ...
  //    for (
  //      gradeType <- getGradeTypes(gradeState) if null != getState(gradeType) && Grade.Status.NEW != getState(gradeType).getStatus &&
  ...
  //    var gradeTypes = getAttribute("gradeTypes").asInstanceOf[Seq[GradeType]]
  //    if (null == gradeTypes) {
