...
class LessonSearchHelper extends SearchHelper {
  ...
  def buildQuery(applyRestriction: Boolean): OqlBuilder[Lesson] = {
    ...
    QueryHelper.populateConditions(query, "lesson.schedule.status,lesson.id")
    ...
    if (null != buildingId || null != courseUnit || null != weekday || 
      ...
      val activityQuery = new StringBuilder("exists( from lesson.schedule.activities as courseActivity where 1=1")
    ...
    if (Strings.isNotEmpty(courseScheduleStatus)) {
      query.where("lesson.schedule.status = :scheduleStatus", CourseScheduleBean.CourseStatusEnum.valueOf(courseScheduleStatus))
    }
    val weeks = Params.getInt("fake.weeks")
    if (null != weeks) {
      query.where("week_state_weeks(lesson.schedule.weekState.value,1)=:weeks", weeks)
    }
    val weekHour = Params.getFloat("fake.weekHour")
    if (null != weekHour) {
      query.where("floor(lesson.course.period / (lesson.schedule.endWeek - lesson.schedule.startWeek  + 1 )) = :weekHour", 
        weekHour.intValue())
    }
    ...
    if (null != lessThan_StartWeek) {
      query.where("lesson.schedule.startWeek >= :lessThanStartWeek", lessThan_StartWeek)
    }
    ...
    if (Strings.isNotBlank(adminclassName)) {
      ...
      if (Collections.isNotEmpty(adminclasses)) {
        ...
        adminclassCondition.append("exists (").append("select litem.id from lesson.teachClass.limitGroups lgroup join lgroup.items litem where")
          ...
          .append(LessonLimitMeta.Adminclass.getMetaId)
          .append(" and ")
    ...
    if (Strings.isEmpty(Params.get("orderBy"))) {
      ...
      } else if ("lesson.schedule.weeks asc" == Params.get("orderBy")) {
        query.orderBy("(lesson.schedule.endWeek - lesson.schedule.startWeek + 1) asc")
      } else if ("lesson.schedule.weeks desc" == Params.get("orderBy")) {
        query.orderBy("(lesson.schedule.endWeek - lesson.schedule.startWeek + 1) desc")
    ...
    val limitItemConditions = Collections.newBuffer[Any]
    val educationId = Params.getInt("limitGroup.education.id")
    if (null != educationId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.Education, educationId))
    }
    val stdTypeId = Params.getInt("limitGroup.stdType.id")
    if (null != stdTypeId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.StdType, stdTypeId))
    }
    val departId = Params.getInt("limitGroup.depart.id")
    if (null != departId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.Department, departId))
    }
    val majorId = Params.getInt("limitGroup.major.id")
    if (null != majorId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.Major, majorId))
    }
    val directionId = Params.getInt("limitGroup.direction.id")
    if (null != directionId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.Direction, directionId))
    }
    val genderId = Params.getInt("limitGroup.gender.id")
    if (null != genderId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.Gender, genderId))
    }
    val programId = Params.getInt("limitGroup.program.id")
    if (null != programId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.Program, programId))
    }
    val stdLabelId = Params.getInt("limitGroup.stdLabel.id")
    if (null != stdLabelId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.StdLabel, stdLabelId))
    }
    val normalClassId = Params.getInt("limitGroup.normalClass.id")
    if (null != normalClassId) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMetaEnum.NORMALCLASS, normalClassId))
    }
    val grade = Params.get("limitGroup.grade")
    if (StringUtils.isNotBlank(grade)) {
      limitItemConditions.add(new Pair[LessonLimitMetaEnum, Any](LessonLimitMeta.Grade, grade))
    }
    if (Collections.isNotEmpty(limitItemConditions)) {
  ...
  private def limitGroupCondition(limitItemConditions: List[Pair[LessonLimitMetaEnum, Any]]): String = {
